...
use common::oep5and8::{owner_of, transfer_oep5, balance_of_oep4, transfer_oep4};
use ostd::abi::Sink;
...
pub fn set_ontd(bridge: &Address) {
    check_admin();
    put(KEY_NFT_ONTD, bridge);
}

pub fn get_ontd() -> Address {
    get(KEY_NFT_ONTD).unwrap_or_default()
}
...
pub fn ontd_to_wont(from: &Address, to: &Address, amount: U128) -> bool {
    check_sig(from);
    let ontd = &get_ontd();
    let this = address();

    let to_bal_before = balance_of_oep4(ontd, to);
    transfer_oep4(ontd, from, to, amount);
    let to_bal_after = balance_of_oep4(ontd, to);
    assert_eq!(to_bal_after - to_bal_before, amount);
